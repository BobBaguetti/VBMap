...
export async function setupSidebar(
  ...
  gCb.checked = true;  // match initial state of script.js
  ...
  function filterMarkers() {
    ...
    allMarkers.forEach(({markerObj,data})=>{
      ...
      let enemyOK = true;
      if (isNpc) {
        const cb = document.querySelector(`#enemy-filter-list input[data-enemy-id="${data.id}"]`);
  ...
  async function loadItemFilters(){
    ...
    defs.filter(d=>d.showInFilters).forEach(d=>{
      ...
      cb.type="checkbox"; cb.checked=true; cb.dataset.itemId=d.id;
  ...
  async function loadEnemyFilters(){
    ...
    npcs.forEach(d=>{
      const lab=document.createElement("label");
      const cb=document.createElement("input");
      cb.type="checkbox"; cb.checked=true; cb.dataset.enemyId=d.id;
      const sp=document.createElement("span"); sp.textContent=d.name;
      lab.append(cb,sp);
      eList.append(lab);
      cb.addEventListener("change",filterMarkers);
