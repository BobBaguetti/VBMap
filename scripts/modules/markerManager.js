...
export function createPopupContent(m) {
  ...
  if (m.type === "Item") {
    if (m.itemType) itemTypeHTML = `<div style="margin:2px 0;font-size:18px;color:${m.itemTypeColor};">${m.itemType}</div>`;
    if (m.rarity)    rarityHTML   = `<div style="margin:2px 0;font-size:18px;color:${m.rarityColor};">${formatRarity(m.rarity)}</div>`;
    if (m.description)descHTML    = `<p style="margin:2px 0;color:${m.descriptionColor};">${m.description}</p>`;
    m.extraLines?.forEach(line => {
      extraHTML += `<p style="margin:2px 0;color:${line.color};">${line.text}</p>`;
    });
  } else if (m.description) {
    descHTML = `<p style="margin:2px 0;color:${m.descriptionColor};">${m.description}</p>`;
  }

  const nameHTML = `<h3 style="margin:0;font-size:20px;color:${m.nameColor};">${m.name}</h3>`;

  const bigImg = isImgUrl(m.imageBig)
    ? `<img src="${m.imageBig}" class="popup-image" style="width:64px;height:64px;object-fit:contain;background:#222;border:2px solid #777;border-radius:4px;" onerror="this.style.display='none'">`
    : "";

  const videoBtn = m.videoURL
    ? `<button class="more-info-btn" onclick="openVideoPopup(event.clientX,event.clientY,'${m.videoURL}')">Play Video</button>`
    : "";

  return `
    <div class="custom-popup">
      <button class="popup-close-btn" onclick="this.closest('.leaflet-popup').remove()">×</button>
      <div class="popup-header" style="display:flex;gap:5px;">
        ${bigImg}<div style="margin-left:5px;">${nameHTML}${itemTypeHTML}${rarityHTML}</div>
      </div>
      <div class="popup-body">${descHTML}${extraHTML}${videoBtn}</div>
    </div>`;
}
