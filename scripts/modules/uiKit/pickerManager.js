...
/**
 * A small wrapper around global Pickr to standardize theme & components.
 */
export class PickerManager {
    ...
    create(selector, options = {}) {
      ...
      const pickr = Pickr.create(config)
        .on('save', (_instance, picker) => picker.hide());
    ...
    remove(pickr) {
      if (this._pickers.has(pickr)) {
        try { pickr.destroyAndRemove(); } catch {}
