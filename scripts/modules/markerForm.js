...
export function initMarkerForm(db) {
  ...
  function renderLines(readOnly = false) {
    ...
    lines.forEach((ln,i) => {
      ...
      // initialize picker on this button
      const p = mkPicker(`#${clr.id}`);
  ...
  /* ---------- UI Helpers ---------- */
  const DIS_BG = "#3b3b3b";
  const ALL    = [fldName,fldRare,fldIType,fldDescIt,fldImgS,fldImgL,fldVid];
  ...
  function lockFields(on) {
    ALL.forEach(e=>setRO(e,on));
    pickrs.forEach(p=>{
      const r = p.getRoot?.();
      if (r && r.style) { r.style.pointerEvents = on ? "none":"auto"; r.style.opacity = on ? 0.5:1; }
    });
  ...
  ddPre.onchange = () => {
    ...
    if (id) { customMode=false; fillFormFromDef(defs[id]); lockFields(true); }
    else    { customMode=true ; clearFormForCustom();    lockFields(false); }
  };
  ...
  function fillFormFromDef(d) {
    ...
    fldIType.value = d.itemType||d.type; pkItyp.setColor(d.itemTypeColor||"#E5E6E8");
    fldDescIt.value = d.description||""; pkDitm.setColor(d.descriptionColor||"#E5E6E8");
    ...
    if (out.type === "Item") {
      if (!customMode) {
        ...
        addItemDefinition(db,defPayload).then(newDef=>{
          defs[newDef.id]=newDef; ddPre.value=newDef.id; customMode=false;
          fillFormFromDef(newDef); lockFields(true);
        });
  ...
  function openEdit(markerObj,data,evt,onSave) {
    populateForm(data); positionModal(modal,evt); modal.style.display="block";
    if(submitCB) form.removeEventListener("submit",submitCB);
    submitCB=e=>{e.preventDefault();Object.assign(data,harvestForm(data.coords));onSave(data);modal.style.display="none";};
    form.addEventListener("submit",submitCB);
  }
  function openCreate(coords,defType,evt,onCreate) {
    populateForm({type:defType||"Item"}); positionModal(modal,evt); modal.style.display="block";
    if(submitCB) form.removeEventListener("submit",submitCB);
    submitCB=e=>{e.preventDefault();onCreate(harvestForm(coords));modal.style.display="none";};
    form.addEventListener("submit",submitCB);
  }
