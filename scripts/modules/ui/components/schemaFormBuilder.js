...
// @version: 1.0 - generic form controller based on schema
...
import {
  ...
  import { usePickrs } from "./pickrUtils.js";
  ...
  export function createSchemaFormController(schema, { onCancel, onDelete, onSubmit }) {
    ...
    schema.forEach(fieldDef => {
      ...
      // Collect pickr targets
      if (fieldDef.pickr) {
        const btn = row.querySelector(".color-swatch");
        if (btn) pickrButtons[fieldDef.key] = btn;
      }
    ...
    // Pickr API
    const pickrApi = usePickrs(form, pickrButtons);
