...
/**
 * Controller for the quest definition form.
 */
export function createQuestFormController({ onCancel, onSubmit, onDelete }) {
  const { form, fields, subheadingWrap, subheading } = createQuestForm();
  const pickrs = {};
  let _id = null;
  ...
  btnDelete.onclick = () => {
    if (_id && confirm("Delete this quest?")) onDelete?.(_id);
  };
  ...
  function populate(def) {
    ...
    _id = def.id || null;
    subheading.textContent = _id ? "Edit Quest" : "Add Quest";
  }
