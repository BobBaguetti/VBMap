...
// @version: 1.2 – reorder buildShell before buildActions so actionRow exists

export function initCrudModal({ id, title, db, loadAll, subscribeAll, onSave, onDelete, renderEntry, formFactory, previewType = null, layoutOptions = ["row","stacked","gallery"] }) {
    ...
    function buildShell() {
      ...
      shell = createDefinitionModalShell({ id, title, withPreview: !!previewType, previewType, layoutOptions });
      const { header, bodyWrap, previewApi: p, open: shellOpen } = shell;
      previewApi = p;
  
      // Move existing search bar
    ...
    return {
      async open() {
        if (!shell) {
          buildForm();
          buildShell();    // <-- now buildShell before buildActions
          buildActions();
          buildList();
          startSubscription();
          await refreshList();
