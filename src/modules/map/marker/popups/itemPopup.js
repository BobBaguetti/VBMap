...
// @version: 1.3 — nest close button inside header for CSS Grid
...
export function renderItemPopup(m) {
  ...
  const typeHTML   = m.itemType
    ? `<div class="popup-type" style="color:${m.itemTypeColor || defaultNameColor};">${m.itemType}</div>`
    : "";
  const rarityHTML = m.rarity
    ? `<div class="popup-rarity" style="color:${m.rarityColor || defaultNameColor};">${formatRarity(m.rarity)}</div>`
    : "";
  ...
  const descHTML   = m.description
    ...
    : "";
  const extraHTML  = (m.extraLines||[]).map(l =>
